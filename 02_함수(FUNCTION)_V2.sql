...
/*
    선택함수
        * DECODE(비교대상(컬럼/연산식/함수식), 비교값1, 결과값1, 비교값2, 결과값,....)
        ...
        자바의 SWITCH와 비슷함
*/
-- 사번, 사원명, 주민번호, 성별 조회 (1:'남', 3:'남', 2:'여', 4:'여', '알수없음)
...
/*
    직급이 J7인 사원은 10% 인상
          ...
          그외        5% 인상
*/
SELECT EMP_NAME 이름, SALARY 기존급여
    , SALARY * DECODE(JOB_CODE, 'J7', 1.1, 'J6', 1.15, 'J5', 1.2, 1.05) 인상급여
FROM EMPLOYEE;
...
SELECT TO_CHAR(SUM(SALARY), '999,999,999L') 총급여 FROM EMPLOYEE;
...
SELECT MIN(EMP_NAME) 문자타입, MIN(SALARY) 숫자타임, MIN(HIRE_DATE) 날짜타입
FROM EMPLOYEE;
...
/*
    ...
    [표현법] MAX(모든타입)
*/
SELECT MAX(EMP_NAME) 문자타입, MAX(SALARY) 숫자타임, MAX(HIRE_DATE) 날짜타입
FROM EMPLOYEE;
...
/*
    ...
    [표현법]
            ...
            COUNT(DISTINCT 컬럼) : 해당 컬럼값의 중복을 제거한 후의 행의 갯수를 세어 반환
                            --> 중복제거시 NULL은 포함하지 않고 갯수가 세어짐
*/
-- 전체 사원 수 조회
SELECT COUNT(*) 전체사원수 FROM EMPLOYEE;

--남자사원수 조회
SELECT COUNT(*) 남자사원수 FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) IN (1,3);
...
-- 부서 배치받은 사원 수 조회 => 부서코드가 NULL이 아닌 행의 개수
