/*
    ...
    DCL(CONTROL, 데이터 제어) : GRANT, REVOKE(권한회수)
...
/*
    ...
    -> 테이블의 모든 컬럼에 대한 값을 제시하여 저장(추가)
        컬럼순서에 맞게 나열해야함(해당 컬럼의 데이터 타입에 맞게)
*/
...
/*
    ...
    ->  컬럼을 직접 제시하여 해당 컬럼에 대한 값만을 추가
        제시되지 않은 컬럼에 대한 값은 기본적으로 NULL값이 저장되고,
            기본값에 대한 옵션이 있는 경우 해당 기본값으로 저장됨.
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, EMAIL, JOB_CODE)
    VALUES (301, '양준혁', '000501-3000000', 'junhyoek2@or.kr', 'J7');
...
-- DEPT_ID가 'D1'인 부서의 부서명을 '인사팀'으로 변경
UPDATE DEPT_TABLE 
    SET DEPT_TITLE = '인사팀'
     WHERE DEPT_ID = 'D1';

-- 부서코드가 d9인 부서의 부서명읠 '전략기획팀' 으로 변경
UPDATE DEPT_TABLE 
    SET DEPT_TITLE = '전략기획팀'
     WHERE DEPT_ID = 'D9';

-- 사원 테이블을 EMP_TABLE로 복제 (사번, 이름, 부서코드, 급여, 보너스정보만)
...
WHERE DEPT_ID = 'D1';
--> D1 값을 EMPLOYEE 테이블 사용 중이기 때문에 삭제 불가 ( 외래키 사용)
