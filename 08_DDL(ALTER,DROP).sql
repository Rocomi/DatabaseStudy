...
-- * 크기를 35바이트 -> 10바이트로 변경
-- ALTER TABLE DEPT_TABLE MODIFY DEPT_TITLE VARCHAR(10);  --> 크기오류 발생!

-- * 크기를 35바이트 -> 50바이트로 변경
ALTER TABLE DEPT_TABLE MODIFY DEPT_TITLE VARCHAR(50);

-- SALARY 컬럼을 VARCHAR로 바꾸면 형식 오류 발생
...
ALTER TABLE DEPT_TABLE 
    ...
    ADD CONSTRAINT DT_UNIQUE UNIQUE(DEPT_TITLE)
    MODIFY LNAME NOT NULL;
/*
    [제약조건 삭제]
    ALTER TABLE 테이블명 DROP CONSTRINT 제약조건명 / NOT NULL 은 예외
    
    ALTER TABLE 테이블명 NODIFY 컬럼명 NULL
*/
ALTER TABLE DEPT_TABLE DROP CONSTRAINT DT_PK;

ALTER TABLE DEPT_TABLE
    DROP CONSTRAINT DT_UNIQUE
    MODIFY LNAME NULL;
--------------------------------------------------------------------------------
-- 테이블 삭제 
...
--                 3) DROP TABLE 테이블명 CASCADE CONSTRAINT
--------------------------------------------------------------------------------
...
-- 2) 제약조건명 변경 : RENAME CONSTRAINT 기존제약조건명 TO 바꿀조건명
ALTER TABLE DEPT_TABLE RENAME CONSTRAINT SYS_C008576 TO DT_DEPTID_NN;
