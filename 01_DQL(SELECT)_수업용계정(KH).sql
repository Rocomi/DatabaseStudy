...
*/
-- 사원 이름, 연봉 정보 조회
-- SALARY*12 : SALARY 컬럽 데이터에 12를 곱하여 조회
-- AS 는 테이블 이름을 원하는 문자열로 표기함 AS ""
SELECT EMP_NAME AS "사원이름" , SALARY*12 AS "연봉" , SALARY AS "급여"
FROM EMPLOYEE;

-- 사원이름, 급여, 보너스, 연봉, 보너스 포함 연봉 (급여 + (급여*보너스)) * 12 정보를     조회
SELECT EMP_NAME AS "사원이름" , SALARY AS "급여", BONUS AS "보너스" , SALARY*12 AS "연봉" , (SALARY + (SALARY * BONUS))*12 AS "연봉(보너스포함)" 
FROM EMPLOYEE;
...
/*
    * 컬럼명 별칭 지정 : 연산식을 사용한 경우 의미파악이 어렵기 때문에, 별칭을 부여하고 명확하고 깔끔하게 조회가능!
    [표현법] 1) 컬럼명 별칭
            2) 컬럼명 AS 별칭
            3) 컬럼명 "별칭"
            4) 컬럼명 AS "별칭"
...
SELECT * FROM employee      -- 전체컬럼을 조회할 건데 , EMPLOYEE 테이블에서
WHERE DEPT_CODE = 'D9';     -- DEPT_CODE의 값이 'D9'인 정보만 조회

-- 사원테이블에서 부서코드가 'D1' 인 사원들의 사원명,급여,부서코드 조회
SELECT EMP_NAME AS 사원명, SALARY AS 급여, DEPT_CODE AS 부서코드 FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';

-- 부서코드가 D1이 아닌 사원들의 정보를 조회 (사원명, 급여, 부서코드)
SELECT EMP_NAME AS 사원명, SALARY AS 급여, DEPT_CODE AS 부서코드 FROM EMPLOYEE
WHERE DEPT_CODE != 'D1';
...
-- 급여가 350만원 이상이고 600만원 이하인 사원의 사원명, 사번, 급여 조회 (BETWEEN AND o)
SELECT EMP_NAME , EMP_NO, SALARY FROM EMPLOYEE
WHERE SALARY BETWEEN 3500000 AND 6000000;

/*
    * NOT : Oracle 에서의 논리부정연산자
*/
...
-- 부서코드가 D6이거나 D8이거나 D5인 사원들의 사원명, 부서코드, 급여를 조회 (IN O)
SELECT EMP_NAME , DEPT_CODE, SALARY FROM EMPLOYEE
...
/*
    * LIKE : 비교하고자 하는 컬럼의 값이 제시한 특정 패턴에 만족할 경우 조회
        => 특정 패턴 : '%', '_'를 와일드카드로 사용
            ...
            * '_' : 1글자
                ex) 비교대상컬럼명 LIKE '_문자' => 비교대상컬럼의 값에서 문자 앞에 무조건 "한글자"가 올 경우 조회
                    비교대상컬럼명 LIKE '__문자' => 비교대상컬럼의 값에서 문자 앞에 무조건 "두글자"가 올 경우 조회
...
-- 사원들 중 이메일에 _ 가 있으면 해당 문자 앞이 3글자인 사원의 사번, 이름, 이메일 조회
...
/*
    * ORDER BY : SELECT문의 가장 마지막 줄에 작성, 실행순서 또한 마지막에 실행
    ...
    [표현법]
        ...
        FROM 테이블명
        WHERE 조건식
        ORDER BY 정렬기준이 되는 컬럼 | 별칭 | 컬럼 순번 [ASC|DESC] [NULLS FIRST|NULLS LAST]
...
-- ORDER BY BONUS ASC;  기본값 (NULLS LAST)
-- ORDER BY BONUS ASC NULLS LAST;

SELECT *
FROM EMPLOYEE 
ORDER BY BONUS DESC; -- 기본값 (NULLS FIRST)
