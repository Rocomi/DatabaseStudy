...
/*
    ...
    - 자료형 
            * 문자 => 반드시 크기 지정을 해줘야 함!!!!!!!!!
                ...
                + VARCHAR2(바이트크기)   : 가변길이 (에디터의 길이가 정해져있지 않은 경우)
...
COMMENT ON COLUMN MEMBER.JOIN_DATE IS '날짜';

-- 테이블 삭제하기 : DROP TABLE 테이블명;
-- DROP TABLE MEMBER;

-- 테이블에 데이터 추가하기 Insert into 
Insert INTO MEMBER VALUES (1,'kauli','1234', '엄희윤', '남', '010-7440-0000', 'kauli1014@naver.com', SYSDATE);

Insert INTO MEMBER VALUES (2,'POWERDOWN','1234', '기다운', '남', NULL, NULL, SYSDATE);
...
Insert INTO MEMBER_NOTNULL VALUES (1,'kauli','1234', '엄희윤', '남', '010-7440-0000', 'kauli1014@naver.com', SYSDATE);

Insert INTO MEMBER_NOTNULL VALUES (2,'POWERDOWN','1234', '기다운', NULL, NULL, NULL, NULL);

Insert INTO MEMBER_NOTNULL VALUES (2,NULL,'1234', '기다운', NULL, NULL, NULL, NULL);

Insert INTO MEMBER_NOTNULL VALUES (NULL,NULL,NULL, NULL, NULL, NULL, NULL,NULL);
-- > 설정한 대로 NOT NULL제약을 걸어준 컬럼은 NULL값이 입력되지 않음 (제약조건 위배)

Insert INTO MEMBER_NOTNULL VALUES (1,'kauli','1234', '엄희윤', '남', '010-7440-0000', 'kauli1014@naver.com', SYSDATE);
--> 중복되는 데이터가 있음에도 추가가 잘 되었다....(곤란타...)
...
/*
    ...
    * 데이터 추가/수정 시 기존에 있는 데이터 값 중에 중복되는 값이 있을 경우 오류 발생시킴!
*/
CREATE TABLE MEMBER_UNIQUE (
    ...
    EMAIL VARCHAR2(50),
    JOIN_DATE DATE    
--  , UNIQUE(해당컬럼명)  -- 테이블 레벨 방식
);

SELECT * FROM MEMBER_NOTNULL;

Insert INTO MEMBER_UNIQUE VALUES (1,'kauli','1234', '엄희윤', '남', '010-7440-0000', 'kauli1014@naver.com', SYSDATE);

Insert INTO MEMBER_UNIQUE VALUES (2,'kauli','1245', '희윤엄', NULL, NULL, NULL, NULL);
...
VALUES (1,'kauli','1234', '엄희윤', '남', '010-7440-0000', 'kauli1014@naver.com', SYSDATE);
...
INSERT INTO MEMBER_UNIQUE 
VALUES (3,'kauli','1234', '희윤엄', NULL, NULL, NULL, NULL);
--> CONSTRAINT 에 쓴 제약조건이 출력되며 오류를 표시해줌

COMMIT;
--------------------------------------------------------------------------------
/*
    CHECK(조건식) : 해당 컬럼에 저장할 수 있는 값에 대한 조건을 제시
                    조건에 만족하는 값만을 저장할 수 잇음
                    => 정해진 갑만을 저장하고자 할 때 사용
*/
...
INSERT INTO MEMBER_CHECK
VALUES(2, 'wan', '1234', '허완', '남성', NULL, NULL, SYSDATE);
--> 성별 컬럼이 체크 제약조건에 위배됨!

INSERT INTO MEMBER_CHECK
VALUES(2, 'wan', '1234', '허완', '남', NULL, NULL, SYSDATE);
--> 체크 조건에 맞는 값만 저장 가능!
...
--> 회원번호(MEM_NO)는 동일하나 회원 아아티(MEM_ID)값이 다르기 떄문에 추가가 됨!

SELECT * FROM MEMBER_PRI2;

-- 어떤 회원이 제품을 당바구니에 담는 정보를 저장하는 테이블
...
/*
    FOREINGN KEY (외래키)
    : 다른 테이블에 존재하는 값을 저장하고자 할 때 사용되는 제약 조건
        -> 다른 테이블을 참조한다고 표현
...
--    , FOREIGN KEY(MEM_GARDEID) REFERENCES MEMBER_GARDE(GRADE_NO) --테이블레벨방식
);
...
--> 외래키로 설정된 컬럼에는 기본적으로 null값을 저장 가능!
...
DELETE FROM MEMBER_GRADE
WHERE GRADE_NO = 300;
--> 자식 테이블에서 300이라는 값을 사용하지 않았기 때문에 삭제가 가능하다
...
/*
    외래키 제약조건 삭제 옵션
    : 부모테이블의 테이터 삭제시 해당 데이터를 사용하고 있는 자식테이블의 값을
            어떻게 할 것인지에 대한 옵션
    ...
    - ON DELETE CASCAED : 부모테이블에 있는 데이터 삭제시 해당 데이터를 사용중인 자식테이블의 값도 삭제
*/
DROP TABLE MEMBER;
...
/*
    DEFAULT
    : 제약조건은 아님..
        컬럼을 선정하지 않고 데이터 추가시 NULL값이 아닌 기본적인 값을 저장하고자 할 때
*/
...
INSERT INTO MEMBER VALUES (2, '맹구', 5, '콧물돌리기',NULL);
...
/*
    * ALTER TABLE : 테이블에 변경사항을 적용하고자 할 때 사용
    ...
    - ----
    - DEFAULT 옵션 : ALTER TABLE 테이블명 NODIFY 컬럼명 DEFAULT 기본값;
*/
