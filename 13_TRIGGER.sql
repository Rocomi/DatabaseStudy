/*
    * 트리거 (TRIGGER)
        : 지정한 테이블에 DML(INSERT, UPDATE, DELETE) 문에 의해 변셩사항이 있을 때(테이블에 이벤트가 발생했을 때)
...
/*
    * 트리거의 종류 *
    ...
    - SQL문에 의해 영향을 받는 각 행에 따른 분류
        ...
        + 행 트리거     : 해당 SQL 문이 실행될때마다 매번 트리거 실행
...
/*
    * 트리거 생성하기
    ...
    CREATE [OR REPLACE] TRIGGER 트리거명                -- 트리거 기본정보(이름)
    BEFOR|AFTER INSERT|UPDATE|DELETE ON 테이블명        -- 이벤트 관련 정보
    ...
    /
*/
-- EMPLOYEE 테이블에 테이터가 추가될 때마다 '신입사원님 환영합니다. ^^' 출력
...
INSERT INTO TB_PRODUCT(PNO, PNAME, BRAND)
    VALUES (SEQ_PNO.NEXTVAL, '아이폰15', '사과');
INSERT INTO TB_PRODUCT
    VALUES (SEQ_PNO.NEXTVAL, '갤럭시 폴드6', '샘송', 2000000, 50);
INSERT INTO TB_PRODUCT
    VALUES (SEQ_PNO.NEXTVAL, '샤오미폰', '샤우미', 80000, 500);
...
/*
    * TB_PDETAIL (입출고이력) 테이블에 데이터 추가(INSERT) 이벤트 발생 시
      TB_PRODUCT (상품) 테이블에 재고 수량이 생긴(UPDATE) 되어야 함! => 자동으로 처리할 수 있도록 트리거 생성!
